...
from suspicion_calculator import SuspicionCalculator
...
def run():
    ...
    for bug_file, bug_data in bug_coverage_data_map.items():
        ...
        function_count_matrix_map = function_count_matrix_map.loc[(
            function_count_matrix_map != 0).any(axis=1)]
        function_count_matrix_map.to_excel(os.path.join(
            FUNCTION_RES_SAVE_PATH, bug_file+"_function_count_matrix_map.xlsx"))


def get_suspicion_matrix():
    """
    获取可疑度矩阵
    """

    # 读取可疑度矩阵
    bugs = os.listdir(FUNCTION_RES_SAVE_PATH)
    # 去除文件夹
    bugs = [bug for bug in bugs if "." in bug]
    for bug in tqdm(bugs):
        # if bug.split("_")[0] != "1":
        #     continue
        print("start to calculate bug: ", bug)
        bug_file_name = bug.split("_")[0]
        # 测试用例名称*测试用例函数（前100），值为运行次数
        bug_function_count_matrix_map = pd.read_excel(os.path.join(
            FUNCTION_RES_SAVE_PATH, bug), index_col=0)
        # 计算可疑度，调用每一个计算公式，保存新的excel。
        # 名称bug_file_name+"_" + methond_name + "suspicion_matrix_map.xlsx"
        suspicionCalculator = SuspicionCalculator(
            bug_name=bug_file_name, save_path=FUNCTION_SUSPICION_RES_SAVE_PATH)
        suspicionCalculator.calculate_all_methods(
            bug_function_count_matrix_map)
...
FUNCTION_SUSPICION_RES_SAVE_PATH = "/root/kratos_testcase/coverage/coverage_process/function_res/suspicion_res"
